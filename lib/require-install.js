module.exports = function(_package, repository) {
	...
	this._isRelativeModule = function(_package) {
		if (_package.substr(0, 1) === '@') {
			return false;
		}
		if (_this._isRepositoryUrl(_package)) {
			throw new Error("To require install a package by repository, pass the repository url as the second argument or in package.json");
		}
		return _package.indexOf('/') >= 0;
	}
	...
	this._tryRequire = function(_package) {
		...
		try {
			...
			if (_this._isRelativeModule(_package)) {
				throw e;
			}
	...
	this._install = function(_package) {
		...
		var version = _this.package_version(_package);
	...
	this.package_version = function(_package) {
		var dep_keys = ['devDependencies', 'dependencies'];
		for (var dep in dep_keys) {
			var dependencies = _this.package_config().get(dep_keys[dep]);
